...
class IteratedBlowup(SageObject):
    ...
    def __init__(self, exact_stratum, global_profile, level, base, blowup_centers):
        ...
        self.divisors = self.global_profile.building_set(self.level)[-_sage_const_1 ]
    ...
    def sub_blowup(self, exc_bd):
        ...
        step_adjustment = {-_sage_const_1 :-_sage_const_1 }
        new_centers = []
        for i, center in enumerate(self.blowup_centers):
            ...
            if not center.are_disjoint(exc_bd):
                ...
                step_adjustment[i] = len(new_centers)-_sage_const_1 
...
#         return nb, nb[-1]
    ...
    def aluffi_decomposition(self, deg, undeg, step):
        ...
        if intermediate != deg:
            ...
            return remainder_nb * recursive_nb, remainder_nb[-_sage_const_1 ]* recursive_fc
        else: 
            return remainder_nb, remainder_nb[-_sage_const_1 ]
    ...
    def building_transform(self, step):
        ...
        return self.proper_transform(bd, step-_sage_const_1 )
    ...
    def final_blowup_difference(self, exc_bd):
        return self.proper_transform(exc_bd,len(step)-_sage_const_1 )
    ...
    def proper_transform(self, exc_bd, step):
        if step == -_sage_const_1 :
            nb = exc_bd.formal_normal_bundle(self.base, mult = True)
            return nb, nb[-_sage_const_1 ] 
        else:
            nb_old, fc_old = self.proper_transform(exc_bd,step-_sage_const_1 )
    ...
    def proper_transform_diff(self, exc_bd, step):
        ...
        if step == -_sage_const_1 :
            return DivTautPolynomial([_sage_const_0 ]),_sage_const_0 
        
        
        assert step >=_sage_const_0  and step< self.num_of_blowups, str(step) + 'is no in range ' + str(self.num_of_blowups)
        ...
        if center.are_disjoint(exc_bd):
            ...
            # Z - ambient space
            cXW, fXW = self.aluffi_decomposition(intersection, center, step-_sage_const_1 )
            cXY, _ = self.aluffi_decomposition(intersection, exc_bd, step-_sage_const_1 )
            cWZ, _ = self.aluffi_decomposition(center, base, step-_sage_const_1 )
            cherns = [cXY, cXW, cWZ]
...
#         print("Computing the class of " + str(self.basic_exact_stratum) +" without blowing up.")
        ...
        for bic_index in building_divs.keys():
            ...
            div_class -= bic.formal_normal_bundle(mult = True)[-_sage_const_1 ]
            
#         print("Line bundle class: ", div_class)
        rank = self.global_profile.ranks[-self.level]
        
        # Total Chern of the proper transform of A_P^{[0]}
        chern = DivTautPolynomial([_sage_const_1 ,div_class])**rank
        fc = chern[-_sage_const_1 ]
#         print("Fundamental class: ", fc)

        diff = sum(exc_bd.formal_normal_bundle(self.base, mult  = True)[-_sage_const_1 ] for exc_bd in self.blowup_centers)
...
#             print('Mult: ',self.X.bics[exc_bd.profile[0]].ell )
    ...
    def zero_locus_blowdown(self):
            return self.blowdown(self.zero_locus_chern()[_sage_const_0 ]), self.blowdown(self.zero_locus_chern()[-_sage_const_1 ])
    ...
    def blowdown(self, divTaut):
        result = divTaut
        reverse_centers = self.blowup_centers[::-_sage_const_1 ]
    ...
    def _blowdown_step(self, step, divTaut):
        ...
        coeff = [ (-_sage_const_1 )**(k-_sage_const_1 )* aj* segre_k(k - codim)                  for aj, k in divTaut.coefficients(E)]

        # The constant term stays the same
        coeff[_sage_const_0 ] = divTaut.coefficients(E)[_sage_const_0 ][_sage_const_0 ]
        return sum(coeff)
    ...
    def aluffi_proper_transform(self, cherns, codims, E, fXW):
        ...
        excess = cWZ.divide(cXY, d-e)
        segre = [(-E)**i for i in (ellipsis_range(_sage_const_1 ,Ellipsis,d-e))]
        
        return - sum(excess[d-e-k]*Xbar*segre[k-_sage_const_1 ] for k in (ellipsis_range(_sage_const_1 ,Ellipsis,d-e)))
    ...
    def aluffi_formal_tensor(self, A,B, E):
        ...
        for i, term in enumerate(result):
            result[i] = sum( coeff* E**power for coeff,power in term.coefficients(E)[_sage_const_1 :])
