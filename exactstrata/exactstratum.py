...
class ExactStratum(SageObject):
    ...
    def __init__(self, X, bundle):
        ...
        self.residue_rank = sum(max(0, len(signature.pole_ind)-1) for signature in self.X._sig_list)
    ...
    def torsion_class(self):
        result = 1
        for index in [1..self.torsion_rank]:
...
#             bic_poles = [leg for leg in bic_poles if BIC.LG.levelofleg(leg)==0 ]
            ...
            for v, _ in enumerate(LG.vertex_list()):
    ...
    def _residue_ranks(self,pole_list):
        ...
        def residue_rank(index):
            ...
            show(pole_list)
            residue_conditions = 0
            for i, sig in enumerate(X._sig_list):
...
#         assert type(i) is sage.rings.integer.Integer, str(i) + " is not an integer but has type "+ str(type(i))
    ...
    def __repr__(self):
        return str([[sig.sig for sig in self.X._sig_list], self.bundle])  
    ...
    def bundle_basis(self):
        ...
        num_marked_points =  sum([len(sig.sig) for sig in self.X._sig_list]) #number of marked points
